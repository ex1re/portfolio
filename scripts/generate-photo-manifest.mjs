...
try {
  ...
  console.warn('photo manifest: exif-reader unavailable, camera settings will be blank')
}
...
/**
 * ISO, aperture and shutter, as the camera recorded them.
 *
 * `npm run trim` keeps the tags when it cuts a web master, so this reads them
 * off the file in the repo and nobody has to type them in. A photo that arrived
 * without them — a scan, an export that stripped metadata — simply reports
 * nothing, and the entry in projects.ts can fill the gap by hand.
 */
async function settingsOf(absPath) {
  if (!sharp || !exifReader) return null
  try {
    const { exif } = await sharp(absPath).metadata()
    if (!exif) return null
    const tags = exifReader(exif)?.Photo
    if (!tags) return null
    const iso = Array.isArray(tags.ISOSpeedRatings) ? tags.ISOSpeedRatings[0] : tags.ISOSpeedRatings
    const seconds = tags.ExposureTime
    const out = {}
    if (iso) out.iso = String(iso)
    // Trailing zeros read as a misprint on a lens: f/2, not f/2.0.
    if (tags.FNumber) out.aperture = String(Number(tags.FNumber.toFixed(1)))
    if (seconds) out.shutter = seconds >= 1 ? `${Number(seconds.toFixed(1))}s` : `1/${Math.round(1 / seconds)}`
...
async function describe(absPath) {
  ...
  const settings = await settingsOf(absPath)
  return { src: preview ?? fullSrc, fullSrc, width, height, ...(settings ? { settings } : {}) }
}
